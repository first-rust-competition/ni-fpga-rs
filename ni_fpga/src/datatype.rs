...
pub type Func<T> = unsafe fn(ffi::Session, ffi::Offset, T) -> ffi::Status;

pub trait Datatype: Copy {
    const FFI_READ: Func<*mut Self>;
    const FFI_WRITE: Func<Self>;
...
impl Datatype for bool {
    const FFI_READ: Func<*mut Self> = |session, offset, target| unsafe {
        ffi::ReadBool(session, offset, target)
    };
    const FFI_WRITE: Func<Self> = |session, offset, value| unsafe {
